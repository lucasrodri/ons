 ...
 * @author lucas
 */
public class MBM_SBVT implements RA {
    ...
    static int conta = 0;
    /**
     * is the number of sub-transponders on each SBVT
     */
    static int subTransponders = 3;
    ...
    private LightPath createLightPathInOpticalGrooming(ArrayList<Integer> path, int rate, LightPath candidate) {
        LightPath lp = null;
        long id;
        if(cp.getVT().getTunnel(candidate.getID()).size() >= subTransponders){
            return null;
        }
