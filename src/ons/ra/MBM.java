 ...
 * @author lucas
 */
public class MBM implements RA {
    ...
    public void flowArrival(Flow flow) {
        ...
        while (true) {
            ...
            lp_aux = FPA(flow, modulation, ksp);//uso de FPA classico
            if (lp_aux != null) {
                lightpaths.add(lp_aux);
                lps = arrayListLPToArrayLP(lightpaths);
                cp.acceptFlow(flow.getID(), lps);
            ...
            for (ArrayList<Integer> path : kpaths) {
                ...
                while (candidate != null) {
                    ...
                    if (lp_aux != null) {
                        ...
                        lp_aux = createLightPathInOpticalGrooming(overPath, flow.getRate(), modulation, candidate);
                    }
            ...
            if (srcNode == flow.getSource()) {
                //ele vai tentar com a modulacao do while externo
                lp_aux = KSP(flow, flow.getSource(), modulation, ksp);//uso de KSP classico
                if (lp_aux != null) {
                    lightpaths.add(lp_aux);
                    lps = arrayListLPToArrayLP(lightpaths);
                    cp.acceptFlow(flow.getID(), lps);
                ...
                //ele vai tentar com todas as modulacoes ate a modulacao do while externo
                int modulation_aux = EONPhysicalTopology.getMaxModulation();
                while (true) {
                    lp_aux = KSP(flow, srcNode, modulation_aux, ksp);//uso de KSP classico
                    if (lp_aux != null) {
                        lightpaths.add(lp_aux);
                        lps = arrayListLPToArrayLP(lightpaths);
                        cp.acceptFlow(flow.getID(), lps);
     ...
     * The KSP algorithm.
     * @param flow the flow
     * @param srcNode the source node. Note in this case the flow's srcNode is not applicable
     * @param modulation the modulation format
     * @param ksp the ksp using in YenKSP
     * @return the lightpath.
     */
    private LightPath KSP(Flow flow, int srcNode, int modulation, int ksp) {
        LightPath lightpath = null;
        int[] nodes, links;
        long id;
        ArrayList<Integer>[] kpaths = YenKSP.kShortestPaths(graph, srcNode, flow.getDestination(), ksp);
     ...
     * @param flow the flow
     * @param modulation the modulation format
     * @param ksp the ksp, if you use the YenKSP
     * @return the lightpath
     */
    private LightPath FPA(Flow flow, int modulation, int ksp){
        LightPath lp = null;
        // k-Shortest Paths routing disjoint first
        //PASSO 1
        ArrayList<Integer>[] kpaths = DSP.disjointShortestPaths(graph, flow.getSource(), flow.getDestination());
        ...
        for (ArrayList<Integer> path : kpaths) {
            ...
            if (modulationPath(path, modulation)) {
                ...
                while (candidate != null) {
                    //PASSO 4                    
                    lp = createLightPathInOpticalGrooming(path, flow.getRate(), modulation, candidate);
                    //PASSO 5
                    if (lp != null) {
                        return lp;
                    } else {
    ...
    /**
     * Retrieves if the modulation format supports the path's distance.
     * @param path the path in nodes
     * @param modulation the id of modulation format
     * @return true if modulation format supports the path's distance, false otherwise
     */
    ...
    /**
     * The KSP algorithm using multihop based in modulation format. Falta terminar
     * @param flow the flow
     * @param srcNode the source node. Note in this case the flow's srcNode is not applicable
     * @param modulation the modulation format
     * @param ksp the ksp using in YenKSP
     * @return the lightpath.
     */
    private ArrayList<LightPath> KSPMultihop(Flow flow, int srcNode, int modulation, int ksp) {
        LightPath lightpath = null;
        int[] nodes, links;
        long id;
        ArrayList<Integer>[] kpaths = YenKSP.kShortestPaths(graph, srcNode, flow.getDestination(), ksp);
