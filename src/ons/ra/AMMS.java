 ...
 * @author lucas
 */
public class AMMS implements RA {
    ...
    private double getFragmentation(int requiredSlots, int enlace) {
        return 1 - ( (double) requiredSlots * (double) ((EONLink) cp.getPT().getLink(enlace)).rangeFreeSimultaneous(requiredSlots)) / (double) ((EONLink) cp.getPT().getLink(enlace)).getAvaiableSlots();
    }

    private boolean aproveFragmentation(ArrayList<Integer> kPath, double fragmentationLevel, int requiredSlots) {
        int[] links = new int[kPath.size() - 1];
        for (int i = 0; i < kPath.size() - 1; i++) {
            links[i] = cp.getPT().getLink(kPath.get(i), kPath.get(i+1)).getID();
        }
        for (int i = 0; i < links.length; i++) {
            if(getFragmentation(requiredSlots, links[i]) >= fragmentationLevel ){
                return false;
            }
        }
        return true;
    ...
    private ArrayList<ArrayList<Integer>> removeGLfromKPath(ArrayList<Integer>[] kPaths, ArrayList<ArrayList<Integer>> grayList) {
        ArrayList<ArrayList<Integer>> list = new ArrayList<>();
        for (ArrayList<Integer> kPath : kPaths) {
            if(!grayList.contains(kPath)){
                list.add(kPath);
            }
        }
        return list;
